...
Generator::Generator(const PrintObject &print_object, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    const unsigned int         filament_id          = region_config.sparse_infill_filament_id.value;
    ...
    const double               nozzle_diameter      = print_config.nozzle_diameter.get_at(tool_id);
...
//    const int                  infill_extruder      = region_config.infill_extruder.value;
    ...
    m_infill_extrusion_width = scaled<float>(region_config.sparse_infill_line_width.get_at(config_index).get_abs_value(nozzle_diameter));
    ...
    if (m_infill_extrusion_width < EPSILON)
        m_infill_extrusion_width = scaled<float>(
            object_config.line_width.get_at(config_index).get_abs_value(nozzle_diameter) < EPSILON ?
            default_infill_extrusion_width :
            object_config.line_width.get_at(config_index).get_abs_value(nozzle_diameter)
        );
