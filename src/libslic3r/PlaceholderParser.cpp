                ...
                if (boost::ends_with(opt_key, "line_width")) {
    			    ...
    			    for (;;) {
    			        ...
    			        if (boost::ends_with(opt_def->ratio_over, "line_width")) {
                    		// Line width supports defaults and a complex graph of dependencies.
							assert(opt_parent->type() == coFloatOrPercent || opt_parent->type() == coFloatsOrPercents);
							v *= Flow::extrusion_width(opt_def->ratio_over, opt_parent, *ctx, static_cast<unsigned int>(ctx->current_extruder_id));
                        	break;
                        }
