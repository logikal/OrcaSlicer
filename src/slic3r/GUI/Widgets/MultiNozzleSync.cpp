    ...
    : DPIDialog(parent, wxID_ANY, _L("Set nozzle count"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    auto update_nozzle_error = [this, force_no_zero, content, max_nozzle_count](int standard_count, int highflow_count) {
        ...
        if (0 < total_count && total_count <= max_nozzle_count && m_error_label->IsShown()) {
            ...
            const wxString error_tip = total_count == 0 ? _L("Error: Can not set both nozzle count to zero.")
                                                        : wxString::Format(_L("Error: Nozzle count can not exceed %d."), max_nozzle_count);
...
{
    // "Direct Drive TPU High Flow" is offered only on the H2D/H2D Pro 0.4 and 0.6 nozzles.
    constexpr double kEps = 0.01;
    return std::fabs(nozzle_diameter - 0.4) < kEps || std::fabs(nozzle_diameter - 0.6) < kEps;
}
...
bool extruder_supports_tpu_high_flow(PresetBundle *preset_bundle, int extruder_id)
{
    ...
    return nozzle_diameter != nullptr && extruder_id >= 0 && extruder_id < (int) nozzle_diameter->values.size() &&
           nozzle_diameter_supports_tpu_high_flow(nozzle_diameter->values[extruder_id]);
}
